...
**什么时候打开：** agent 在当前批分镜/参考图完成并自检后主动打开，说明待审段号和检查重点；样片完成后再打开视频对照。看完直接回到对话回复通过范围或按镜号提出修改。页面验收框只用于导出，下载笔记也不会自动发给 agent。完整阶段交接见 [审阅交接](review-handoff.md)。
...
输出路径必须不存在，避免覆盖已写笔记的报告。打开 HTML 默认进入「多格故事板」，不用先点击预览按钮。

1. **选择片段，看画面顺序**：从左到右、从上到下看多格图；可调整每行格数。
2. **点某一格，检查这一镜**：下方自动选中对应镜头。展开拍摄要求，对照动作、景别、运镜和连续性；填写返工笔记，或用本地图片替换。图片立即更新到多格图，不会自动修图。
3. **按用途下载**：审阅图给人看，含镜号和拍摄说明；视频参考图给模型，无新增说明文字，需逐格检查并勾选确认；单格下载在当前镜头旁。镜头映射 JSON 在高级选项内。

需要查参数、比对生成视频时，切换「镜头明细与视频对照」。这里可按集数、场景、段号或动作筛选，点击时间带定位镜头，按时长顺序预览，切换同段不同任务版本。两个视图共用返工笔记，顶部「下载返工笔记」输出 JSON。顶部「保存编辑会话」下载包含笔记、替换图片、片段和布局的 JSON；重新打开同一份报告，用「恢复会话 / 合并笔记」恢复。关闭前需手动保存，不会自动回写项目或审批。
...
页面直接显示可点击的单格预览；下载区的「查看最近一次导出的实际排版」可展开检查 PNG 排版。支持以下导出：
...
若图与镜头内容不符，可用“替换选中单格”加载本机 PNG/JPEG/WebP，只更新当前页面的导出素材，不覆盖项目源图。替换后需重新检查；关闭前请保存编辑会话，或导出 PNG。变化后的源文件需要在项目中单独登记并重新审批。
...
编辑会话只保存替换图片和笔记，原始素材仍由原报告提供。会话按报告快照哈希匹配；分镜、素材或生产状态变化后，应打开原报告恢复，不能强行套到新报告。恢复不会继承验收勾选，图片需重新检查。会话仅记录当前片段、镜头和列数，不保存视频播放状态或明细筛选。

恢复会话会替换当前编辑；若当前存在未保存修改，先保存再恢复。笔记文件采用合并：同一镜头已有不同笔记时整次导入失败，避免覆盖意见。旧版笔记没有快照哈希，只核对标题和镜头标识，页面会明确提示。未知镜头、重复条目、格式错误或损坏图片均拒绝导入，失败不改变当前内容。

替换图片单张上限 30 MB、总计 96 MB，JSON 文件上限 140 MB。会话包含图片内容，分享时按素材的分享范围处理。导出 PNG 或仅下载笔记不等于保存整个会话；有未保存内容时关闭页面会提示。浏览器已开始下载后视作已保存，请保留下载文件。
