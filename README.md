...
`novel-script` 的创作检查默认采用 `reviewPolicy: advisory`：台词长度、钩子/悬念、公式时长等显示建议，结构与引用错误仍拦截。需要维持旧版全部硬门的项目，应按已确认约定在 script.json 显式设置 `reviewPolicy: strict`。其他专业工具仍有旧版创作门，使用时须说明适用性，不为消除告警静默改戏。下面是阶段依赖示意，不是每份材料必走的全流程。
