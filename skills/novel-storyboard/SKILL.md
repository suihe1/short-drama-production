...
| 单镜头 ≤ 15 秒 | AI 视频单段生成上限，长对话在生成层按模型能力分段，是否切镜服从导演意图（`params.maxShotSeconds` 按模型改） |
...
已有剧本可先做导演与分镜讨论，不要求先改写大纲。使用本工具 validate/render/export 时需要结构化 script.json 和 `--script`；先忠实映射原稿，保留对白、事件与已有拍法，无法承载的意图另附交接说明。其余上游按有则用：
