...
const advisories = gateReport(doc).filter(g => !g.ok);
assert.ok(advisories.some(g => g.id === 'duration'));
assert.ok(advisories.some(g => g.id === 'line-length'));
assert.ok(advisories.some(g => g.id === 'hook-cliff'));
assert.ok(advisories.every(g => g.severity === 'warning'));
assert.ok(renderHtml(doc).includes('项创作建议'));
assert.ok(renderHtml(doc, { lang: 'en' }).includes('advisories;'));
assert.equal(JSON.stringify(doc), before, 'validation and rendering must not rewrite content');
assert.ok(validateScript({ ...doc, reviewPolicy: 'strict' }).length >= 4);
