...
默认 `reviewPolicy: advisory`（省略时同样生效）。时长估算、单条对白长度、钩子/悬念、开场位置、动作表述和爽点认领属于创作建议；保留诊断，不阻止交付初稿。结构错误、无效参数、说话人和已提供资产的引用错误仍会失败。
...
当前估时不支持声画重叠，cliff 字段也仍沿用旧名：独立片可以留空，并用正文说明结尾完成了什么；advisory 下只提示。不要为了去掉提示虚构下一集悬念。技术通过不代表故事好看、内容获批或时长实测通过。
