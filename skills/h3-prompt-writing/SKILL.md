...
Consume the approved shot plan, reference roles and requested duration; do not redesign the story or approve source media. Draft rewrites may identify missing inputs explicitly, but final references and labels must refer to actual reviewed assets. Return prompts to the storyboard/production owner; prompt completion does not authorize generation or certify video quality. Keep the vendor reference documents unchanged. The bundled guides target 4–15 seconds; a provider advertising longer output does not itself extend this prompt contract. Route longer requests to the production adapter's compatibility workflow instead of silently stretching timestamps.
